...
describe('fusionOrder', () => {
  it('higher score wins regardless of everything else', () => {
    const low = candidate({ score: 0.01, bm25Rank: 1, vectorRank: 1 })
  ...
  it('on tied scores, a both-pass candidate beats a single-pass candidate', () => {
    const both = candidate({ bm25Rank: 62, vectorRank: 62, bm25Score: 1, vectorScore: 0.5 })
    const single = candidate({ bm25Rank: 1, bm25Score: 9 })
  ...
  it('on tied single-pass scores, the vector-side candidate beats the keyword-side one', () => {
    const keywordOnly = candidate({ bm25Rank: 1, bm25Score: 9, external_id: 'kw' })
  ...
  it('both-pass candidates with mirrored ranks fall back to raw scores', () => {
    const strongerKeyword = candidate({ bm25Rank: 1, vectorRank: 2, bm25Score: 9, vectorScore: 0.5 })
    const strongerVector = candidate({ bm25Rank: 2, vectorRank: 1, bm25Score: 5, vectorScore: 0.9 })
